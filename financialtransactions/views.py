...
class FinancialTransactionList(APIView):
    ...
    def post(self, request):
        try:
            ...
            x = 0
            if request.data["presentation_id"]:
                x += 1
            if request.data["selected_presentation_id"]:
                x += 1
            if request.data["selected_exam_id"]:
                x += 1
            if x > 1:
                return Response(
...
class FinancialTransactionDetail(APIView):
    ...
    def delete(self, request, pk):
        try:
            ...
            return Response({"massage": "financial transaction deleted"}, status=status.HTTP_204_NO_CONTENT)
        except FinancialTransaction.DoesNotExist:
            return Response({"error": "financial transaction not found"}, status=status.HTTP_404_NOT_FOUND)
